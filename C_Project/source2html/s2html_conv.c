...
void get_events(FILE *source_fp, EventStruct *event_struct)
{
	...
	while(1)
	{
		if( event_struct->source_fdata[fdata_idx] == '\0' )
		{	
			event_struct->event_cdata = '\0';
			return;
		}
...
printf("1\n");
			while(1)
			{
				...
				event_struct->source_fdata[fdata_idx + i] = '\5';
printf(" %d\t", i + fdata_idx);
//printf("(%c, %c)\t",event_struct->source_fdata[fdata_idx + i], event_struct->source_fdata[fdata_idx + i + 1]);
printf("%c",event_struct->source_fdata[ i + fdata_idx ]);

	if(event_struct->source_fdata[fdata_idx + i + 1] == '*' && event_struct->source_fdata[fdata_idx + i + 2] == '/')
				{
printf("3\n");
					break;
				}
...
//			event_struct->source_fdata[fdata_idx + 0] = '\5';
			...
			for( i = 0, tfdata_idx = 0; event_struct->source_fdata[fdata_idx + i] != '\n'; i++, tfdata_idx++ )
			{
			...
			event_struct->source_tfdata[tfdata_idx + 1] = '\0';
			event_struct->source_fdata[fdata_idx + i] = '\5';
